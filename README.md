...
rewildingproject <- readRDS(file = "/Users/howardng/documents/Rewilding Project/Block 1 + Block 2/objects/rewildingprojectobject.rds")
...
table(rewilding$HTO_classification.global)
table(rewilding$HTO_maxID)
table(rewilding$stim)

rewildingproject <- FindVariableFeatures(rewildingproject, selection.method = "vst", nfeatures = 5000)
top10RNA <- head(VariableFeatures(rewilding), 10)
View(top10RNA)
plot1 <- VariableFeaturePlot(rewilding)
plot2 <-LabelPoints(plot = plot1, points = top10RNA, repel = TRUE)
plot2

rewildingproject <- FindVariableFeatures(rewildingproject, assay = "ADT")
top10ADT <- head(VariableFeatures(rewilding$ADT), 10)
View(top10ADT)
plot1 <- VariableFeaturePlot(rewilding, assay = "ADT")
